...
class _LoaningTabState extends State<LoaningTab> {
  ...
  Future<void> _performDeleteLoanPlan(int index) async {
    try {
      ...
      // Check if there are any active loans using this plan
      ...
      if (activeLoansCount.count > 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Cannot delete loan plan. There are $activeLoansCount active loans using this plan.',
            ),
